...
os.makedirs("./iterationsNORMAL", exist_ok=True)

mean1, sd1 = 25, 8.3
mean2, sd2 = -25, 8.3
def sample_mixture(n, dim):
    mask = np.random.rand(n, dim) < 0.5
    X = np.empty((n, dim))
    X[mask] = np.random.normal(loc=mean1, scale=sd1, size=np.sum(mask))
    X[~mask] = np.random.normal(loc=mean2, scale=sd2, size=np.sum(~mask))
    return X


s = time.time()
for i in range(10):
        iter_dir = f'./iterationsNORMAL/iter_{i}'
        tree_dir = os.path.join(iter_dir, 'tree')
        os.makedirs(iter_dir, exist_ok=True)
        os.makedirs(tree_dir, exist_ok=True)
        
        #######################
        Xa = sample_mixture(100, 3)
        Xb = sample_mixture(50, 3)
        Ya = sample_mixture(100, 3)
        Yb = sample_mixture(50, 1)
